...
def offload(source):
    ...
    ec_phys_driver = ec_phys_fc['EC_PHYS_FC']

    driver_calls = tuple(
        call for call in FindNodes(CallStatement).visit(ec_phys_driver.body)
        if '_LAYER' in str(call.name)
    )
    driver_loops = tuple(
        loop for loop in FindNodes(Loop).visit(ec_phys_driver.body)
