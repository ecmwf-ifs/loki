...
class IntrinsicTypeHip(IntrinsicTypeCpp):
    """
    Mapping Fortran type to corresponding HIP type.
    """
    # pylint: disable=unnecessary-pass
    pass

hip_intrinsic_type = IntrinsicTypeHip()


class HipCodeMapper(CppCodeMapper):
    """
    A :class:`StringifyMapper`-derived visitor for Pymbolic expression trees that converts an
    expression to a string adhering to standardized HIP.
    """
    # pylint: disable=abstract-method, unused-argument, unnecessary-pass
    pass


class HipCodegen(CppCodegen):
    """
    Tree visitor to generate standardized HIP code from IR.
    """
    ...
    def __init__(self, depth=0, indent='  ', linewidth=90, **kwargs):
        symgen = kwargs.pop('symgen', HipCodeMapper(hip_intrinsic_type))
