...
from loki.types import AllocatedName, BasicType, DerivedType
...
class FortranMaxTransformation(Transformation):
    ...
    def _generate_dfe_kernel(self, routine, **kwargs):  # pylint: disable=unused-argument
        ...
        cond_map = {}
        cond_type = AllocatedName(BasicType.LOGICAL)
            ...
            for v in FindVariables(unique=False).visit(max_kernel.ir):
                ...
                if isinstance(v, sym.Array) and v.dimensions is not None:
                    ...
                    if str(index) == dim:
                        ...
                        fct_symbol = sym.ProcedureSymbol('stream.offset', scope=max_kernel.scope)
            ...
            if stmt.lhs.type.dfevar:
                if isinstance(stmt.rhs, (sym.FloatLiteral, sym.IntLiteral)):
                    _type = sym.InlineCall(sym.ProcedureSymbol('%s.getType' % stmt.lhs.name,
                                                               scope=max_kernel.scope))
                    rhs = sym.InlineCall(sym.ProcedureSymbol('constant.var', scope=max_kernel.scope),
        ...
        def base_type(var_type):
            ...
            if var_type.dtype == BasicType.LOGICAL:
                return sym.InlineCall(sym.ProcedureSymbol('dfeBool', scope=max_kernel.scope))
            if var_type.dtype == BasicType.INTEGER:
                # TODO: Distinguish between signed and unsigned
                return sym.InlineCall(sym.ProcedureSymbol('dfeUInt', scope=max_kernel.scope),
            ...
            if var_type.dtype == BasicType.REAL:
                ...
                return sym.InlineCall(sym.ProcedureSymbol('dfeFloat', scope=max_kernel.scope),
        ...
        def init_type(var_type):
            ...
            return sym.InlineCall(sym.ProcedureSymbol(name, scope=max_kernel.scope), parameters=parameters)
        ...
        for var in max_kernel.variables:
            if var.type.dfevar:
                if var.type.intent and var.type.intent.lower() == 'in':
                    ...
                    initial = sym.InlineCall(sym.ProcedureSymbol(name, scope=max_kernel.scope),
                                             parameters=parameters)
    ...
    def _generate_dfe_kernel_module(kernel, **kwargs):  # pylint: disable=unused-argument
        ...
        arg_type = AllocatedName(DerivedType('KernelParameters'), intent='in')
    ...
    def _generate_dfe_manager(kernel, **kwargs):  # pylint: disable=unused-argument
        ...
        params_type = AllocatedName(DerivedType('EngineParameters'), intent='in')
    ...
    def _generate_slic_interface(routine, kernel, **kwargs):  # pylint: disable=unused-argument
        ...
        size_t_type = AllocatedName(BasicType.INTEGER, intent='in')  # TODO: make this size_t
