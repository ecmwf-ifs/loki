...
from loki.transformations.utilities import recursive_expression_map_update, get_integer_variable
    ...
    for v in FindVariables(unique=False).visit(routine.body):
        if isinstance(v, sym.Array):
            ...
            for i, d in enumerate(v.shape):
                if is_explicit_range_index(d) and not (isinstance(v.dimensions[i], sym.RangeIndex) and v.dimensions[i].start is None):
...
class LowerConstantArrayIndices(Transformation):
            ...
            for i, dim in enumerate(call_arg.dimensions):
                if self.is_constant_dim(dim):
                    ...
                    if not isinstance(dim, sym.RangeIndex):
                        ...
                        print(f"[LowerConstantArrayIndices] call {call} dim {dim} | '{dim.name.lower()}' vs. ['klev', 'jtile']")
                        if False: # dim.name.lower() in ['klev', 'jtile']:
                            print(f"     yes!")
                            offset_map[routine_arg.name][i] = (None, dim, call_arg.shape[i])
                            current_index -= 1
                        else:
    ...
    def process_callee(self, routine, offset_map):
        ...
        for var_name in offset_map:
            ...
            new_dims = ()
            # print(f"var.name {var.name} vs. {offset_map[var.name]}")
            if not offset_map[var.name]:
                continue
            if not [k for k, v in offset_map[var.name].items() if v != 0]:
                continue
                ...
                for i in range(max(k for k, v in offset_map[var.name].items() if v != 0) + 1):
                    ...
                    if offset is not None:
                        if original_index is None:
                            if isinstance(offset, sym.Scalar):
                                # new_dims += (offset.clone(scope=routine),)
                                if offset.name not in routine.variable_map:
                                    print(f"ADDING {offset.name} to ROUTINE {routine.name}")
                                new_dims += (get_integer_variable(routine, offset.name),)
                            else:
                                new_dims += (offset,)
                        else:
