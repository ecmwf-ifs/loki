    ...
    * Pass the stack arrays and sizes into the kernel calls
    ...
    def int_type(self):
        return SymbolAttributes(
            dtype=BasicType.INTEGER, kind=DeferredTypeSymbol('JWIM')
        )
    ...
    def create_stacks_kernel(self, routine, stack_dict, successors):
        ...
        for dtype in stack_dict:
            for kind in stack_dict[dtype]:
                ...
                # if stack_used_var not in routine.variables:
                #     routine.variables += (stack_used_var,)
                #     routine.body.prepend((Assignment(lhs=stack_used_var, rhs=IntLiteral(0)),))
    ...
    def _map_temporary_array(self, temp_array, int_var, routine, stack_var):
        ...
        for t in temp_arrays:
            ...
            # stack_size_name = self._get_stack_int_name('K', t.type.dtype, t.type.kind, 'STACK_SIZE')
            # stack_size_var = Scalar(name=stack_size_name, scope=routine)
            # relevant_shape = (self._get_horizontal_variable(routine), stack_size_var)
            ...
            if call.name in successor_map and self._key in successor_map[call.name].trafo_data:
                ...
                for dtype in successor_stack_dict:
                    for kind in successor_stack_dict[dtype]:
                        ...
                        ###
                        arg_map = dict(call.arg_iter())
                        print(f"arg_map: {arg_map}")
                        print(f"---------------")
                        print(f"successor_stack_size: {successor_stack_size}")
                        expr_map = {
                            expr: DetachScopesMapper()(arg_map[expr]) for expr in FindVariables().visit(successor_stack_size)
                            if expr in arg_map
                        }
                        print(f"expr_map: {expr_map}")
                        if expr_map:
