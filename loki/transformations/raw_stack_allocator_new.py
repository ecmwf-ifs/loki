...
__all__ = ['TemporariesRawStackNewTransformation']

one = IntLiteral(1)


class TemporariesRawStackNewTransformation(Transformation):
    """
    ...
    * Pass the stack arrays and sizes into the kernel calls
    ...
    def __init__(
            ...
            driver_horizontal=None, int_kind='JPIM', **kwargs
    ):
    ...
    def transform_subroutine(self, routine, **kwargs):
        ...
        if role == 'kernel':
            ...
            stack_arg_dict = self.create_stacks_kernel(routine, stack_dict, successors)

        if role == 'driver':

            stack_dict = self._determine_stack_size(routine, successors, item=item)
            stack_arg_dict = self.create_stacks_driver(routine, stack_dict, successors)
    ...
    def create_stacks_kernel(self, routine, stack_dict, successors):
        ...
        for dtype in stack_dict:
            for kind in stack_dict[dtype]:
                ...
                stack_used_arg = Scalar(name=stack_used_arg_name, scope=routine, type=self.int_type.clone(intent='INOUT'))
                stack_used_name = self._get_stack_int_name('J', dtype, kind, 'STACK_USED')
                stack_used_var = Scalar(name=stack_used_name, scope=routine, type=self.int_type)
                ...
                stack_args += [stack_size_var, stack_var.clone(dimensions=stack_type.shape, type=stack_var.type.clone(contiguous=True)), stack_used_arg] 
    ...
    def apply_raw_stack_allocator_to_temporaries(self, routine, item=None):
        ...
        temporary_arrays = self._find_temporary_arrays(routine)
        ...
        for (dtype, kind_dict) in temporary_array_dict.items():
            ...
            for (kind, arrays) in kind_dict.items():
                ...
                stack_var = self._get_stack_var(routine, dtype, kind)
                old_int_var = stack_used_var # IntLiteral(0)
                old_array_size = ()
    ...
    def _find_temporary_arrays(self, routine):
        """
    ...
    def _sort_arrays_by_type(self, arrays):
        ...
        # for a in arrays:
        #     # type_dict.setdefault(a.type.dtype, []).append(a)
        #     type_dict.setdefault(a.type.dtype, {}).update({}.setdefault(a.type.kind, []).append(a))
