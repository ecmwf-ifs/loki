...
__all__ = ['SCCDemoteVerticalTransformation']


class SCCDemoteVerticalTransformation(Transformation):
    """
    ...
    def __init__(self, vertical, demote_local_arrays=True):
        self.vertical = vertical

        self.demote_local_arrays = demote_local_arrays

    @classmethod
    def get_locals_to_demote(cls, routine, vertical):
        """
        ...
        * Have the ``horizontal`` as the innermost dimension, with all
        ...
        # Only demote local arrays with the vertical as second dimension 
        candidates = [
            v for v in candidates if v.shape and
            len(v.shape) > 1 and v.shape[1] in vertical.sizes
        ]
 
        vertical_loops = [loop for loop in FindNodes(ir.Loop).visit(routine.body)
                if loop.variable.name.lower() == vertical.index.lower()]
        
        # Create an index into all variable uses per vector-level section
        vars_per_loop = {
            s: set(
                v.name.lower() for v in get_local_arrays(routine, s.body, unique=False)
            ) for s in vertical_loops
        }
        ...
        for arr in candidates:
            counts[arr] = sum(
                ...
                for v in vars_per_loop.values()
            )

        # Demote temporaries that are only used in one section or not at all
        # COULD/should be v <= 1, however, variables never used will be removed entirely by another trafo
        ...
        call_args = [
            ...
            if any(s in (p.dimensions or p.shape) for s in vertical.size_expressions)
        ]
    ...
    def transform_subroutine(self, routine, **kwargs):
        ...
        # print(f"[SCCDemoteVertical] called for routine {routine}")
        if role == 'kernel':
    ...
    def process_kernel(self, routine, demote_locals=True, preserve_arrays=None):
        ...
        to_demote = self.get_locals_to_demote(routine, self.vertical)
        ...
        if demote_locals:
            ...
            if variables:
                print(f"[ROUTINE {routine} demote vertical: {variables}]")
                demote_variables(
                    routine, variable_names=variables,
                    dimensions=self.vertical.sizes
                )
