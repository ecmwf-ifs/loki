        ...
        for dtype in stack_dict:
            for kind in stack_dict[dtype]:
                ...
                if dtype in stack_arg_dict:
                    stack_arg_dict[dtype][kind] = (stack_size_var, stack_var.clone(dimensions = (RangeIndex((None,None)), jgpblock,)), stack_used_var)
                else:
                    stack_arg_dict[dtype] = {kind: (stack_size_var, stack_var.clone(dimensions = (RangeIndex((None,None)), jgpblock,)), stack_used_var)}
...
class PoolAllocatorFtrPtrTransformation(PoolAllocatorBaseTransformation):
    ...
    def apply_pool_allocator_to_temporaries(self, routine, item=None):
        ...
        for (dtype, kind_dict) in temporary_array_dict.items():
            ...
            for (kind, arrays) in kind_dict.items():
                ...
                int_var = Scalar(name=self.local_int_var_name_pattern.format(name=f'incr_{self._get_kind_name(kind)}'), scope=routine, type=self.int_type)
                integers += [int_var]
                ...
                for array in arrays:
                    ...
                    ptr_assignment = self._get_ptr_assignment(routine, array, int_var, stack_var)
    ...
    def _get_ptr_assignment(self, routine, array, int_var, stack_var):
        _arr_dim = () 
        _stack_dim_upper = ()
        horizontal_size = self._get_horizontal_variable(routine)
