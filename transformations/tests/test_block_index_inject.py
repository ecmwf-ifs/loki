...
from transformations import (
        BlockViewToFieldViewTransformation, BlockIndexInjectTransformation,
...
@pytest.mark.parametrize('frontend', available_frontends(xfail=[(OMNI,
                         'OMNI correctly complains about rank mismatch in assignment.')]))
def test_simple_lower_loop(blocking, frontend):
...
"""
    ...
    # kernel = Subroutine.from_source(fcode, frontend=frontend)
    BlockIndexLowerTransformation(blocking, recurse_to_kernels=recurse_to_kernels).apply(driver, role='driver', targets=('kernel',))
    BlockIndexLowerTransformation(blocking, recurse_to_kernels=recurse_to_kernels).apply(kernel_mod['kernel'], role='kernel', targets=('compute',))
    BlockIndexLowerTransformation(blocking, recurse_to_kernels=recurse_to_kernels).apply(nested_kernel_mod['compute'], role='kernel')
    BlockIndexInjectTransformation(blocking).apply(driver, role='driver', targets=('kernel',))
    BlockIndexInjectTransformation(blocking).apply(kernel_mod['kernel'], role='kernel', targets=('compute',))
    BlockIndexInjectTransformation(blocking).apply(nested_kernel_mod['compute'], role='kernel')

    BlockLoopLowerTransformation(blocking).apply(driver, role='driver', targets=('kernel',))
    BlockLoopLowerTransformation(blocking).apply(kernel_mod['kernel'], role='kernel', targets=('compute',))
    BlockLoopLowerTransformation(blocking).apply(nested_kernel_mod['compute'], role='kernel')
