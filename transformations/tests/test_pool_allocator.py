...
def check_stack_created_in_driver(driver, stack_size, first_kernel_call, num_block_loops,
                                  generate_driver_stack=True, kind_real='jprb', simplify_stmt=True):
    ...
    if generate_driver_stack:
        assert assignments[1].lhs == 'ylstack%u' and (
               assignments[1].rhs == f'ylstack%l + istsz * max(c_sizeof(real(1, kind={kind_real})), 8)')
    else:
        assert assignments[1].lhs == 'ylstack%u' and (
               assignments[1].rhs == f'ylstack%l + max(c_sizeof(real(1, kind={kind_real})), 8)*istsz')
...
END MODULE
    ...
    fcode_stack_assign = """
        ...
        ylstack%u = ylstack%l + max(c_sizeof(real(1, kind=jprb)), 8) * istsz
    """
...
contains
    ...
    assert transformation._key in kernel_item.trafo_data
    # pylint: disable-next=line-too-long
    assert kernel_item.trafo_data[transformation._key]['stack_size'] == f'max(c_sizeof(real(1, kind={kind_real})), 8)*klon + max(c_sizeof(real(1, kind={kind_real})), 8)*klev*klon + 2*max(c_sizeof(int(1, kind={kind_int})), 8)*klon + max(c_sizeof(logical(true, kind={kind_log})), 8)*klev'
    # pylint: disable-next=line-too-long
    assert kernel2_item.trafo_data[transformation._key]['stack_size'] == f'3*max(c_sizeof(real(1, kind={kind_real})), 8)*klev*klon'
    assert all(v.scope is None for v in
                               FindVariables().visit(kernel_item.trafo_data[transformation._key]['stack_size']))
    assert all(v.scope is None for v in
    ...
    stack_size = f'max(max(c_sizeof(real(1, kind={kind_real})), 8)*nlon + ' \
                 f'max(c_sizeof(real(1, kind={kind_real})), 8)*nlon*nz + '
    stack_size += f'2*max(c_sizeof(int(1, kind={kind_int})), 8)*nlon + ' \
                  f'max(c_sizeof(logical(true, kind={kind_log})), 8)*nz,'
    stack_size += f'3*max(c_sizeof(real(1, kind={kind_real})), 8)*nz*nlon)/' \
                  f'max(c_sizeof(real(1, kind=jprb)), 8)'
...
@pytest.mark.parametrize('frontend', available_frontends())
@pytest.mark.parametrize('directive', [None, 'openmp', 'openacc'])
@pytest.mark.parametrize('stack_size_config', [None, 'dict', 'file'])
def test_pool_allocator_temporaries_kernel_nested(frontend, block_dim, directive, stack_size_config):
    if directive == 'openmp':
            ...
            'routine': [{
                ...
                'trafo': {
                    'TemporariesPoolAllocatorTransformation': {
                        'stack-size': f"2*{stack_size}"
                    }
                }
            }]
        }
    elif stack_size_config == "file":
        config_str = f"""
[default]
mode = 'idem'
role = 'kernel'
...
real_kind = 'jwrb'
[[routine.trafo]]
[[routine.trafo.TemporariesPoolAllocatorTransformation]]
stack-size = "4*{stack_size}"
    """.strip()
        (basedir / 'loki_config.config').write_text(config_str)
    else:
        config = {
            'default': {
                'mode': 'idem',
    ...
    if stack_size_config == "file":
        scheduler = Scheduler(paths=[basedir], config=SchedulerConfig.from_file(basedir / 'loki_config.config'),
    ...
    assert transformation._key in kernel_item.trafo_data
    # pylint: disable-next=line-too-long
    assert kernel_item.trafo_data[transformation._key]['stack_size'] == f'max(c_sizeof(real(1, kind={kind_real})), 8)*klon + 4*max(c_sizeof(real(1, kind={kind_real})), 8)*klev*klon + 2*max(c_sizeof(int(1, kind={kind_int})), 8)*klon + max(c_sizeof(logical(true, kind={kind_log})), 8)*klev'
    # pylint: disable-next=line-too-long
    assert kernel2_item.trafo_data[transformation._key]['stack_size'] == f'3*max(c_sizeof(real(1, kind={kind_real})), 8)*columns*levels'
    assert all(v.scope is None for v in
                               FindVariables().visit(kernel_item.trafo_data[transformation._key]['stack_size']))
    assert all(v.scope is None for v in
                               FindVariables().visit(kernel2_item.trafo_data[transformation._key]['stack_size']))
    ...
    if stack_size_config == "dict":
        stack_size = '2*400*nz*nlon / MAX(C_SIZEOF(REAL(1, kind=JWRB)), 8)'
    elif stack_size_config == "file":
        stack_size = '4*400*nz*nlon / MAX(C_SIZEOF(REAL(1, kind=JWRB)), 8)'
    else:
        stack_size = f'max(c_sizeof(real(1, kind={kind_real})), 8)*nlon/max(c_sizeof(real(1, kind=jwrb)), 8) +'
        stack_size += f'4*max(c_sizeof(real(1, kind={kind_real})), 8)*nlon*nz/max(c_sizeof(real(1, kind=jwrb)), 8) +'
        stack_size += f'2*max(c_sizeof(int(1, kind={kind_int})), 8)*nlon/max(c_sizeof(real(1, kind=jwrb)), 8) +'
        stack_size += f'max(c_sizeof(logical(true, kind={kind_log})), 8)*nz/max(c_sizeof(real(1, kind=jwrb)), 8)'
    # can't use 'simplify(...)' on expression that was generated by 'parse_fparser_expression' containing e.g. 'max'
    check_stack_created_in_driver(driver, stack_size, calls[0], 1, kind_real='jwrb',
                                  simplify_stmt=stack_size_config is None)
...
contains
    ...
    assert 'ydstack' in kernel.arguments
    assert 'ydstack' in kernel2.arguments

    calls = FindNodes(CallStatement).visit(driver.body)
    assert calls[0].arguments == ()
    assert calls[0].kwarguments == (('start', 1), ('end', 'nlon'), ('klon', 'nlon'), ('klev', 'nz'),
                                    ('field1', 'field1(:, b)'), ('field2', 'field2(:, :, b)'), ('YDSTACK', 'YLSTACK'))
    assert calls[1].arguments == ('1', 'nlon', 'nlon', 'nz')
    assert calls[1].kwarguments == (('field2', 'field2(:, :, b)'), ('YDSTACK', 'YLSTACK'))
    assert calls[2].arguments == ('1', 'nlon', 'nlon', 'nz', 'field2(:, :, b)', 'YLSTACK')
    assert calls[2].kwarguments == ()
    assert calls[3].arguments == ('1', 'nlon', 'nlon', 'nz')
    assert calls[3].kwarguments == (('field2', 'field2(:, :, b)'), ('YDSTACK', 'YLSTACK'), ('opt_arg', 'opt'))
    assert calls[4].arguments == ('1', 'nlon', 'nlon', 'nz', 'field2(:, :, b)', 'YLSTACK', 'opt')
