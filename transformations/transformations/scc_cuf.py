...
__all__ = ['SccCufTransformation', 'SccCufTransformationNew', 'HoistTemporaryArraysDeviceAllocatableTransformation',
        'HoistTemporaryArraysCstyleTransformation', 'HoistTemporaryArraysPragmaOffloadTransformation',
...
class SccLowLevelLaunchConfiguration(Transformation):
    ...
    def driver_launch_configuration(self, routine, block_dim, targets=None):
            ...
            if loop.variable == block_dim.index or loop.variable in block_dim._aliases:
                ...
                if True:
                    for call in FindNodes(ir.CallStatement).visit(routine.body):
                        ...
                        if call.pragma:
                            ...
                            parameters = ()
                        print(f"mode: '{self.mode}' kernel within! {call.name} | pragmas: {call.pragma} | parameters: {parameters}")
                        ...
                        if self.mode == 'cuf':
                            print(f"chevron: this is happening!")
                            mapper[call] = (call.clone(chevron=(routine.variable_map["GRIDDIM"],
                ...
                if kernel_within:
                    ...
                    if self.mode == 'cuf':
                        ...
                        griddim_assignment = ir.Assignment(lhs=lhs, rhs=rhs)
                        # if self.mode == 'cuf':
                        print(f"mapper[loop] for {loop}: this is happening!")
                        mapper[loop] = (blockdim_assignment, griddim_assignment, ir.Comment(""), loop.body)
                    else:
                        func_dim3 = sym.ProcedureSymbol(name="dim3", scope=routine)
        ...
        if self.mode == 'cuf':
            print(f"this is happening: mapper: {mapper}")
            # for key in mapper:
            #     print(f"key: {key} - {mapper[key]}")
        routine.body = Transformer(mapper=mapper).visit(routine.body)
