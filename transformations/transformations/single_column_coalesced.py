...
class SCCBaseTransformation(Transformation):
    ...
    def check_horizontal_var(cls, routine, horizontal):
        ...
        if horizontal.bounds[0] not in routine.variable_map and len(horizontal._bounds_aliases) < 2:
            raise RuntimeError(f'No horizontal start variable found in {routine.name}')
        if horizontal.bounds[1] not in routine.variable_map and len(horizontal._bounds_aliases) < 2:
    ...
    def process_kernel(self, routine):
        ...
        if self.horizontal.bounds[0] in routine.variables and self.horizontal.bounds[1] in routine.variables:
            ...
            assert len(self.horizontal._bounds_aliases) == 2
